<?php
	...
	echo '<table  class="table table-hover table-bordered" id="sampleTable" width="90%">
		...
		$sql= "select * from faculty where status='registered'";
			...
			while($row=$results->fetch_array()){
				...
				echo '<td id="phone" align="center">'.$row["phone"].'</td>';
                                ...
                                echo '<td id="action" align="center">'
                                    . '<form action="approve.php?regno='.$row["regno"].'" method="post">'
                                            . '<button class="btn btn-primary" type="submit" id="approve" name="approve"><i class="fa fa-fw fa-lg fa-check-circle"></i>Approve</button>&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;</form>'
                                        . '<form action="reject.php?regno='.$row["regno"].'" method="post">'
                                            . '<button class="btn btn-primary" type="submit" id="reject" name="reject"><i class="fa fa-fw fa-lg fa-check-circle"></i>Reject</button></form></td>';
			    echo '</tr>';
			}
		}
		else{
